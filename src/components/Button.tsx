    ...
    return (
      <div>
        {props.type == 'contained' ? (
            <Link to={props.link} className=" ${props.className} p-3 text-xs bg-green-400 text-white flex items-center rounded font-bold uppercase gap-2 justify-center hover:bg-darkGreen-400 transition-colors">
                {props.text}
            </Link>
        ) : (
